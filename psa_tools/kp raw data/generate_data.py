...
=======================================
Reads FoxPro DBF files and outputs dashboard_data.js for the offline HTML dashboard.

Usage:
    python generate_data.py

Output:
    dashboard_data.js  (same folder as this script)

Run this script every time a fresh DBF export is dropped in the folder.
Schedule via Windows Task Scheduler:
    Program:  python
    Args:     "D:\\claude_code\\psa_tools\\kp raw data\\generate_data.py"
    Start in: D:\\claude_code\\psa_tools\\kp raw data
"""
...
DATA_DIR     = os.path.join(SCRIPT_DIR, "2526")   # DBFs live in the 2526 subfolder
OUTPUT_FILE  = os.path.join(SCRIPT_DIR, "dashboard_data.js")
ENCODING     = "cp1252"                            # FoxPro Windows-1252
...
def s(v):
    """Strip a string value; return '' for None."""
...
def d(v):
    """Convert date to ISO string; return '' for None."""
    if v is None:
        return ''
...
def latest_date(rows, field):
    """Find the latest non-null date across rows."""
    dates = [dict(r)[field] for r in rows if isinstance(dict(r).get(field), (datetime.date, datetime.datetime))]
...
def main():
    ...
    # ── Vouch index: which (INV_NO, CO_CODE, CUST_CD) are settled ────────────
    ...
    for r in vouch_rows:
        ...
        if billfg == 'Y':
            # This bill has been settled — record the key
    ...
    # ── Bills (billmain) — classify outstanding vs settled ───────────────────
    ...
    for r in bm_rows:
        ...
        # Outstanding = not yet in vouch
        is_outstanding = (inv_no, co_code, cust_cd) not in settled_keys

        # Ageing in days (from invoice date, else entry date)
    ...
    print(f"  Settled bills    : {sum(1 for bills in bills_by_customer.values() for b in bills if not b['outstanding']):,}  |  "
          f"Total settled: Rs.{total_settled:,.0f}")

    # ── Payment days from hisablst ────────────────────────────────────────────
    print("Computing payment days from hisablst...")
    # payment_days[cacode+co] = list of (days, bill_date, payment_date, bill_no, amount)
    ...
    # Index indent header by (INDT_ABBR, INDT_NO)
    indm_idx = {}
    for r in indm_rows:
    ...
    # Group indent lines by (INDT_ABBR, INDT_NO) first
    ...
    for (abbr, no), header in indm_idx.items():
        ...
        orders_by_customer[cust_cd].append(order)
    ...
    for code, info in customers.items():
        ...
        # Average payment days (only from hisablst where dddate is set)
        ...
        # Monthly sales trend (last 12 months from billmain)
        ...
        # Unique suppliers this customer deals with
    ...
    payload = {
        "meta": {
            ...
            "bill_count":     sum(len(bm_rows) for _ in [1]),
        },
